...
const UserDonations = () => {
    ...
    return (
        <>
            ...
            <section>
                <div className="section-heading">Donation History</div>
                {(auth?.user && donations.length === 0) ? (
                    <p className='text-muted'>
                        No donations found. Please donate to a temple below{' '}
                        <i className="text-primary fa-solid fa-arrow-down"></i>
                    </p>
                ) : (
                                ...
                                {donations.slice(0, 5).map((donation, index) => {
                                    ...
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                {(donation?.is80CertificateRequested === false && !donation.certificate) ? (
                                                    <button
                                                        onClick={() => handleRequestCertificate(donation._id)}
                                                        className='btn btn-theme-primary'
                                                        title="Request 80 Certificate"
                                                    >
                                                    ...
                                                    donation?.certificate ? (
                                                        <div style={{ gap: "6px" }} className='d-flex flex-wrap'>
                                                            <div className="file-preview">
                                                                <a className='btn btn-theme-primary' target="_blank" href={donation.certificate} download>
                                                                    <i className="fa-solid fa-download"></i> Download
                                                                </a>
                                                            </div>
                                                            {(donation.is80CertificateRequested && donation.certificate) ? (
                                                                <p className='mt-1 text-danger'>Certificate Requested again</p>
                                                            ) : (
                                                                <button
                                                                    onClick={() => handleRequestCertificate(donation._id)}
                                                                    className='btn btn-theme-primary'
                                                                    title="Request 80 Certificate Again"
                                                                >
                ...
                {donations.length > 5 && (
                    <div className="text-left mt-4">
                        <button
                            className="btn btn-theme-primary"
                            onClick={() => navigate('/user/past-donations')}
                        >
                            View All Donations
                        </button>
                    </div>
